...
const AnimalPhotos = () => {
    const [AnimalPhotoInfo,setAnimalPhotoInfo] = useState([]);

    const fetchAnimalPhotos  = async () =>{
        const response = await axios.get('/api/photos/')
        setAnimalPhotoInfo(response.data)
    }

    const createAnimalCards = () => {
        return (
            <>
                {AnimalPhotoInfo.map(photo => {

                    if(!photo){
                        return <div>Loading..</div>

                    } else if(photo.category === 'Animals') {
