...
const SunsetPhotos = () => {
    const [SunsetPhotoInfo,setSunsetPhotoInfo] = useState([]);

    const fetchSunsetPhotos  = async () =>{
        const response = await axios.get('/api/photos/')
        setSunsetPhotoInfo(response.data)
    }

    const createSunsetCards = () => {
        return (
            <>
                {SunsetPhotoInfo.map(photo => {

                    if(!photo){
                        return <div>Loading..</div>

                    } else if(photo.category === 'Sunsets') {
