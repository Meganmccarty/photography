...
const AutumnPhotos = () => {
    const [AutumnPhotoInfo,setAutumnPhotoInfo] = useState([]);

    const fetchAutumnPhotos  = async () =>{
        const response = await axios.get('/api/photos/')
        setAutumnPhotoInfo(response.data)
    }

    const createAutumnCards = () => {
        return (
            <>
                {AutumnPhotoInfo.map(photo => {

                    if(!photo){
                        return <div>Loading..</div>

                    } else if(photo.category === 'Autumn') {
