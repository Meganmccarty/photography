...
const SpringPhotos = () => {
    const [SpringPhotoInfo,setSpringPhotoInfo] = useState([]);

    const fetchSpringPhotos  = async () =>{
        const response = await axios.get('/api/photos/')
        setSpringPhotoInfo(response.data)
    }

    const createSpringCards = () => {
        return (
            <>
                {SpringPhotoInfo.map(photo => {

                    if(!photo){
                        return <div>Loading..</div>

                    } else if(photo.category === 'Spring') {
