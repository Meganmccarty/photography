...
const ArthropodPhotos = () => {
    const [ArthropodPhotoInfo,setArthropodPhotoInfo] = useState([]);

    const fetchArthropodPhotos  = async () =>{
        const response = await axios.get('/api/photos/')
        setArthropodPhotoInfo(response.data)
    }

    const createArthropodCards = () => {
        return (
            <>
                {ArthropodPhotoInfo.map(photo => {

                    if(!photo){
                        return <div>Loading..</div>

                    } else if(photo.category === 'Arthropods') {
    ...
    return (
        ...
        >
            <div className="Gallery">
                {createArthropodCards()}
            </div>
        </LightgalleryProvider>
    )
}

export default ArthropodPhotos