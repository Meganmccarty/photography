...
const CaterpillarPhotos = () => {
    const [CaterpillarPhotoInfo,setCaterpillarPhotoInfo] = useState([]);

    const fetchCaterpillarPhotos  = async () =>{
        const response = await axios.get('/api/photos/')
        setCaterpillarPhotoInfo(response.data)
    }

    const createCaterpillarCards = () => {
        return (
            <>
                {CaterpillarPhotoInfo.map(photo => {

                    if(!photo){
                        return <div>Loading..</div>

                    } else if(photo.category === 'Caterpillars') {
    ...
    return (
        ...
        >
            <div className="Gallery">
                {createCaterpillarCards()}
            </div>
        </LightgalleryProvider>
    )
}

export default CaterpillarPhotos