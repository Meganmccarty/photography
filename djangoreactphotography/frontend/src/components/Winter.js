...
const WinterPhotos = () => {
    const [WinterPhotoInfo,setWinterPhotoInfo] = useState([]);

    const fetchWinterPhotos  = async () =>{
        const response = await axios.get('/api/photos/')
        setWinterPhotoInfo(response.data)
    }

    const createWinterCards = () => {
        return (
            <>
                {WinterPhotoInfo.map(photo => {

                    if(!photo){
                        return <div>Loading..</div>

                    } else if(photo.category === 'Winter') {
